...
if(!fs.existsSync(path.join(mainPath,"kdb.zip")) || Number(fs.readFileSync(path.join(mainPath,"latest.txt"))) + (1000*60*24) < Date.now()){
  console.log("Fetching latest database from archive.org...");
  const p1 = request("https://archive.org/download/kahoot-win/full-export-keys.json").pipe(fs.createWriteStream(path.join(mainPath,"kdb-keys.json")));
  console.log("Saving database to kdb.zip...");
  ...
  p1.on("finish",()=>{
    ...
    p.on("finish",()=>{
      console.log("Downloaded database. Extracting...");
      yauzl.open(path.join(mainPath,"kdb.zip"),{lazyEntries: true},(err,zip)=>{
        ...
        zip.on("entry",entry=>{
          if(/\/$/.test(entry.fileName)){
            ...
            zip.openReadStream(entry,(err,stream)=>{
              if(err){return console.log("Failed to extract: " + e);}
              stream.on("end",()=>{
                zip.readEntry();
              });
        ...
        zip.once("end",()=>{
          console.log("Database successfully extracted.");
...
class QuizFinder{
  ...
  async searchKahoot(index){
    ...
    if(!this.parent.options.uuid){
      ...
      let results2 = (!this.ignoreDB && await SearchDatabase(this,this.DBIndex)) || [];
      if(!this.parent){
        this.hax.stop = true;
        return;
      }
...
function ReadDB(item){
  return new Promise((r,j)=>{
    fs.readFile(path.join(mainPath,item),"utf8",(e,d)=>{
      if(e){return j(e);}
      r(JSON.parse(d));
    });
  });
...
async function SearchDatabase(finder,index){
  ...
  let res = [];
  const keys = await ReadDB("kdb-keys.json");
  try{
    for(let i = index || 0;i< index + (DBAmount || 50);i++){
      const item = await ReadDB("json-full/" + keys[i] + ".json");
      if(filt(item)){
        res.push(item);
      }
    }
